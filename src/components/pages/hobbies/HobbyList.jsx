...
import { deleteHobbyRequest, fetchHobbiesRequest, statusHobbyRequest } from "../../../redux/slices/hobbySlice";

const HobbyList = () => {
    const dispatch = useDispatch();
    const { hobbies, actionLoading } = useSelector((state) => state.hobby);

    useEffect(() => {
        dispatch(fetchHobbiesRequest());
    }, [dispatch]);

    const handleStatusToggle = (item) => {
        dispatch(statusHobbyRequest({ id: item._id, isActive: !item.isActive }));
    };

    const handleDelete = (id) => {
        if (window.confirm("Are you sure want to delete")) {
            dispatch(deleteHobbyRequest(id));
        }
    };
