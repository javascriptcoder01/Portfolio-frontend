...
import { deleteLanguageRequest, fetchLanguagesRequest, statusLanguageRequest } from "../../../redux/slices/languageSlice";

const LanguageList = () => {
    const dispatch = useDispatch();
    const { languages, actionLoading } = useSelector((state) => state.language);
    ...
    const handleStatusToggle = (item) => {
        dispatch(statusLanguageRequest({ id: item._id, isActive: !item.isActive }));
    };

    const handleDelete = (id) => {
        if (window.confirm("Are you sure want to delete")) {
            dispatch(deleteLanguageRequest(id));
        }
    };
